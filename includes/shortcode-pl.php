 ...
 * @return void
 */
function noniko_dmc_register_pl_shortcodes() {

	add_shortcode(
		'noniko_daily_meditation_pl',
		'noniko_dmc_render_meditation_pl_shortcode'
	);
}

add_action(
	'init',
	'noniko_dmc_register_pl_shortcodes'
);
 ...
 * @return string
 */
function noniko_dmc_render_meditation_pl_shortcode( $atts ) {
	...
	/*
	 * If no date was supplied, use today's date.
	 */
	if ( '' === $date ) {
	...
	/*
	 * Get meditation from database.
	 */
	$meditation = noniko_dmc_get_meditation_by_date( $date );
	...
	<article class="noniko-dmc-meditation">
		...
		<header class="noniko-dmc-meditation__header">
			...
			<p class="noniko-dmc-meditation__day">
				<?php
				echo esc_html( $meditation->med_day );
				?>
			</p>

			<h2 class="noniko-dmc-meditation__title">
				<?php
				echo esc_html( $meditation->med_title );
				?>
			</h2>
